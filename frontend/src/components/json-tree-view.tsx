...
function JsonNode({
  ...
  const isTypeField = nodeKey === "@type";
  ...
  const isEditable = (val: JsonValue): boolean => {
    return !isExpandable(val) && onStartEditing !== undefined && !isTypeField;
  }
  ...
  const showApproveAllControls = onRemoveField && isExpandable(displayValue) && validationState === ValidationState.PENDING && !isAllChildrenApprovedOrDeleted(displayValue, nodePath) && !isTypeField;
  const rowOpacityClass = isFullyApproved || isFullyDeclined ? "opacity-75" : "";

  // --- Render old value row as a sibling row if needed ---
  const shouldShowOldValueRow =
  ...
  // --- Hide row for declined modifications ---
  if (diffType === DiffType.MODIFIED && validationState === ValidationState.DECLINED) {
    return null;
  }

  return (
    <>
      {/* Old value row (red) as a sibling row above the new value row */}
      {shouldShowOldValueRow && (
        <div
          className={`font-mono text-sm flex items-center py-1 rounded px-2 transition-colors group bg-red-50 border-l-4 border-red-400`}
          style={{ paddingLeft: `${level * 20}px` }}
        >
          {/* Empty space for expand/collapse icon */}
          <div className="w-4 h-4 mr-2 flex items-center justify-center" />
          {/* Diff indicator (red) */}
          <div className="w-2 h-2 mr-2 rounded-full flex-shrink-0">
            <div className="w-full h-full bg-red-500 rounded-full"></div>
          </div>
          {/* Validation state indicator (none for old value) */}
          <div className="w-2 h-2 mr-2 flex-shrink-0" />
          {/* Key */}
          <span className="text-blue-800 font-medium mr-2">{nodeKey}:</span>
          {/* Value (old, strikethrough, red) */}
          <div className="flex-1 flex items-center justify-between">
            <div className="flex items-center">
              <span className="text-red-700 line-through bg-red-100 px-1 rounded">
      ...
      {isExpandable(displayValue) && isExpanded && (
        <div>
          ...
          <div
            className={`text-gray-600 font-mono text-sm${shouldHighlightWholeBlock() ? ' bg-green-50 border-l-4 border-green-400' : ''} ${rowOpacityClass}`}
